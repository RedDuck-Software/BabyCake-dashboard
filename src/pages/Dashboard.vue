              ...
              <div class="mt-3">
                <div id="myTabContent" class="tab-content info-container">
                  <div id="one" class="tab-pane fade p-3 claim-reward" :class="{ 'active show': isActive('one') }">
                    ...
                    <div v-if="claimToken.showTokenClaimer" class="bought-tokens-claimer col">
                      <div v-if="claimToken.claimIsAvailable">
                        <div class="claim-head row">
                          <span
                            >Already claimed: <span class="val">{{ claimToken.alreadyClaimedTokens }}</span> MKAT</span
                          >
                          <div class="pl-3"></div>
                          <span
                            >Total bought: <span class="val">{{ claimToken.totalBoughtTokens }} </span> MKAT</span
                          >
                        </div>
                        <div class="mt-2"></div>
                        <div class="claim-body row">
                          <div>
                            <div>
                              Tokens to claim remains:
                              <span class="val">{{ claimToken.remainsPreSaleTokens }} </span> MKAT
                            </div>
                            <div v-if="claimToken.tokensToClaim > 0">
                              Available to claim:<span class="val"> {{ claimToken.tokensToClaim }} </span> MKAT
                            </div>
                          </div>
                          <div>
                            <button
                          ...
                          <div v-if="claimToken.nextClaimDate != null">
                            Next available claim: {{ claimToken.nextTokensClaimDate.toGMTString() }}
                          </div>
                        </div>
                      </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      setInterval(async function() {
        await this.getBnbReward(
          new MetamaskService(await MetamaskService.createWalletProviderFromType(this.walletProviderType))
        );
      }, 60000);
    } catch (ex) {
      console.log(ex);
      alert("Please, ensure that right network is selected in your wallet provider." + "Must be: BSC Mainnet");
    } finally {
      this.$loading(false);
    }
  },
  ...
  methods: {
    async loadContractInfo(_service) {
      ...
      const maxPayments = !claimInfo.isValue
        ? 0
        : claimInfo.totalTokensAmount.div(claimInfo.periodPaymentAmount).add(1);
      ...
      if (claimInfo.isValue && this.claimToken.claimIsAvailable) {
        ...
        if (this.claimToken.remainsPreSaleTokens.isZero()) this.claimToken.claimIsAvailable = false;
        ...
        this.claimToken.remainsPreSaleTokens = parseFloat(
          utils.formatUnits(this.claimToken.remainsPreSaleTokens, 9)
        ).toFixed(2);
        let nextClaimDate;
        ...
        if (maxPayments.lte(passedPeriodPaymentsCount) || claimInfo.paymentsMade.eq(maxPayments)) {
          ...
          nextClaimDate = passedPeriodPaymentsCount.eq(claimInfo.paymentsMade)
            ? new Date(1000 * claimStart.add(unFreezePeriod.mul(passedPeriodPaymentsCount)).toNumber())
            : null;
        }
      ...
      this.hundredThousandMKATUSD = parseFloat(
        utils.formatUnits(await service.getMkatValueInBUSD(hundredThousandMKAT), 18)
      ).toFixed(2);
